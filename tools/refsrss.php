...
<rss version="2.0">
  <channel>
    ...
    <link>http://w3future.com/weblog/</link>
...
$knownRefs = hashFromProp("o", "http://w3future.com/rdf#referrer");
$excerptRefs = hashFromProp("s", "http://w3future.com/rdf#firstSeenOn");
...
//    if (!isset($knownRefs[$ref])) 
    if (!isset($excerptRefs[$ref]))
    {
      ...
      rdf_addTriple("http://w3future.com".$url, 'http://w3future.com/rdf#referrer', $ref);
      rdf_addTriple($ref, 'http://w3future.com/rdf#firstSeenOn', date("Y-m-d\TH:i:s", time()));
...
foreach ($checkList as $href => $target) {
  ...
  $lines = file($href, "r");
  if (!$lines) continue;
  rdf_addTriple($href, 'http://w3future.com/rdf#online', 'true');
  ...
  if ($linktext[2])
  {
    rdf_addTriple($href, 'http://w3future.com/rdf#linkText', $linktext[2]);
...
//echo "excerpt: ".$found."\n\n";
  rdf_addTriple($href, 'http://w3future.com/rdf#excerpt', $found);
}

$newToday = getList("s", "(p='http://w3future.com/rdf#firstSeenOn') and (o LIKE '" . date("Y-m-d", time()) . "%')");

while (list ($val, $href) = each ($newToday)) {
  $desc = getDescription($href);
  $targets = getList("s", "(p='http://w3future.com/rdf#referrer') and (o='".addslashes($href)."')");
  $target = $targets[0];
  $href = htmlspecialchars($href);
  $title = $desc['http://purl.org/dc/elements/1.1/title'];
  if (!$title) $title = $href;
  $title = htmlspecialchars($title);
  $firstSeen = $desc['http://w3future.com/rdf#firstSeenOn'];
  if ($desc['http://w3future.com/rdf#excerpt'])
  {
    $txt = htmlspecialchars($desc['http://w3future.com/rdf#excerpt']);
