...
if ( typeof(beyondVer) != "number" || beyondVer < 0.95 )
	alert("beyondBrowser requires beyond JS library ver 0.95 or higher");
var beyondBrowserVer = 0.95;
...
_SP.add = function(where, html) {
	if (isUndefined(document.all)) {
		var el = this.element();
		el.innerHTML = select(where, {
			afterBegin: html + el.innerHTML,
			beforeEnd: el.innerHTML + html,
			undefined: "'" + where + "' location not supported"
		});
	} else this.foreach(function(e) { e.insertAdjacentHTML(where, html); });
	return this;
};
