...
function eventStream(element, eventName, noAttach) {
	...
	if ( !noAttach && isDefined(element.addEventListener) ) {
		function pushEvent(event) {
			st.push(event ? event : window.event);
		}
		if ( element.addEventListener(eventName.substr(2), pushEvent, false) ) {
			st.stop = function() {
				element.removeEventListener(eventName.substr(2), pushEvent, false);
			};
			return st;
		}
	}
