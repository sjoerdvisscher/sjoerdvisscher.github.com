var resultStream, compileDebug, codeInstance, nextCont;
function run(source)
{
  ...
  //try 
  {
    ...
    var yld = l();
    var done = l();
    var linesOfCode = ast.compile(js("$.yld"), js("$.done"));
    codeInstance = runnable([start].concat(linesOfCode).concat([
      yld, js("return $"),
      done, js("return;")
    ]));

    compileDebug = document.getElementById("compileDebug");
    compileDebug.firstChild.nodeValue = codeInstance + "\n\r";
    var $ = {};
    $.yld    = {k: yld.id,   $: $};
    $.done   = {k: done.id,  $: $};
    nextCont = {k: start.id, $: $};
...
/**
 * Compile expressions. () -> [LineOfCode]
 */

LiteralExpr.prototype.compile = function(yieldCont, doneCont)
{
  return [
    new Inv(yieldCont, {value: this.value, next: doneCont})
  ];
};

VarExpr.prototype.compile = function(yieldCont, doneCont)
{
  return [
    new Inv(
      js("$.envNode['" + this.token.value + "'].stream"),
      { yld: yieldCont, done: doneCont }
    )
  ];
};

/*VarRefExpr.prototype.compile = function(yieldCont, doneCont)
{
  if (!envNode[this.token.value])
    throw "Undefined variable: " + this.token.debug();
  yield envNode[this.token.value];
...
LetExpr.prototype.compile = function(yieldCont, doneCont)
{
  var tokens = [];
  this.getLocals(tokens);
  var valueExprLabel = l();
  return [
    js("$.envNode['" + tokens[0].value + "'].setStream({$: {envNode: $.envNode}, k: " + valueExprLabel.id + "})"),
    new Inv(doneCont, {}),
    valueExprLabel
  ].concat(this.valueExpr.compile(js("$.yld"), js("$.done")));
};

JoinExpr.prototype.compile = function(yieldCont, doneCont)
{
  var initCode = [];
  
  if (this.locals.length)
  {
    initCode.push(js("$.envNode = NEW($.envNode)"));
    for (var i = 0; i < this.locals.length; i++)
      initCode.push(js("$.envNode['" + this.locals[i].value + "'] = new Node()"));
  }

  if (!this.exprs.length)
    return initCode.concat(new Inv(doneCont, {}));
  ...
  {
    var next = l();
    code = code.concat(this.exprs[i].compile(yieldCont, new Cont(next)));
    code.push(next);
  }
  
  return code.concat(this.exprs[i].compile(yieldCont, doneCont));
};

ArgExpr.prototype.compile = function(yieldCont, doneCont)
{
  return envNode["?"];
};
...
ApplyExpr.prototype.compile = function()
{
  var fnCode = this.fnsExpr.compile();
  var argCode = this.argsExpr.compile();
  return gen(
    fe("fn", fnCode[0].id,
      fe("v", js("$.fn.invoke(" + argCode + ")"),
        yld("$.v")
      )
    )
  ).concat(fnCode).concat(argCode);
};
...
Function.prototype.subClass = function(mNames)
{
  var f = function() {
    ...
    for (var mName in mNames)
      if (mNames.hasOwnProperty(mName))
...
Inv = Object.subClass({lExpr: String, args: Object});
